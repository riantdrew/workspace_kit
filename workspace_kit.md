...
7. **MUST** класть markdown агента **только** в `workspace/`. Правила, хуки, скиллы и команды — создавать адаптер под текущую платформу (Cursor → `.cursor/`, OpenCode → `.opencode/`, MCP → `mcp/`). Ничего из platform-адаптера не класть в `workspace/`.
...
| **1.5** | 19.06.2Alternatively | Платформонезависимая архитектура; session memory (3 слоя + orphan); SOUL.communication; MEMORY.hypotheses; GOALS.guardrails; PLAYBOOK.md; goal lens; agent-knowledge rule |
...
- Что повесить на platform adapter (rules, hooks, skills) — [Интеграции](#интеграции)
...
| Положить SKILL.md в `workspace/skills/` как runtime | Платформа не подхватит | Адаптер под платформу (`.cursor/skills/`, OpenCode-адаптер (если требуется), `mcp/`)|
...
4. **Platform adapter:** `.cursor/rules/`, `.cursor/hooks.json`, `.cursor/skills/`, `.cursorignore` или OpenCode-адаптер (если требуется) — список и краткое содержимое.
...
│   ├── faq/
│   └── references/
├── SKILLS.md               # каталог → platform adapter skills
├── TOOLS.md
├── ENV.md
├── PLAYBOOK.md
...
.cursor/                    # адаптер Cursor — НЕ в workspace/
├── rules/
├── hooks/
...
mcp/                        # адаптер MCP (любой клиент) — НЕ в workspace/
...
| `.opencode/` | OpenCode-специфичные конфигурации (если среда поддерживает) |
| `mcp/*.json` | MCP server configs (любой MCP-клиент) |
| `AGENTS.md` (корень) | Агент ищет в корне — **короткий указатель** |
...
- Skills runtime — **адаптер под платформу** (`.cursor/skills/`, OpenCode-адаптер (если требуется), `mcp/`)
- Каталог skills для людей — `workspace/SKILLS.md` (пути на platform-адаптер)
- Не класть workspace в `docs/`, `src/`, `.cursor/rules/` или адаптеры других платформ (если присутствуют)
...
| Процедуры skills | Адаптер платформы (`skills/`) + индекс `workspace/SKILLS.md` |
...
**Разделы:** `skills` (таблица: имя, триггер, путь к platform adapter skills), `conflict_resolution`
...
Правила, события, скиллы и команды — **платформозависимые механизмы**.  
Кит описывает концепцию; агент определяет среду и создаёт адаптер под неё.
...
### Формат по платформам

Агент определяет среду по файлам проекта и создаёт адаптер:

| Платформа | Путь адаптера | Формат правил |
|-----------|---------------|---------------|
| Cursor | `.cursor/` | `.mdc` (YAML frontmatter + Markdown) |
| OpenCode | `.opencode/` (если среда поддерживает) | формат определяется средой |
| MCP-клиент | `mcp/` | `*.json` (MCP server config) |

### Пример: Cursor

<!-- Шаблоны ниже — для Cursor. Для других платформ — эквивалентные файлы в соответствующем адаптере. -->
...
- Проектные skills — в адаптере платформы (`.cursor/skills/`, OpenCode-адаптер (если требуется), `mcp/`), не в workspace/
```
...
Общие события, реализация зависит от платформы:
...
Этот модуль **не в workspace/** — он часть агентской инфраструктуры, распространяется как artifact kit через `pilot-sync/`.

### Skills

| Уровень | Где лежит | Кто подхватывает |
|---------|-----------|------------------|
| Платформенные | `~/.cursor/skills/`, OpenCode-платформенные (если требуется) | Среда исполнения |
| **Проектные** | Адаптер платформы (`.cursor/skills/`, OpenCode-адаптер (если требуется), `mcp/`) | Среда исполнения |
...
| Команда | Назначение | Платформа |
|---------|------------|-----------|
| `workspace/BOOTSTRAP.md` | Ритуал сессии | Любая |
| `workspace/GOALS.md`, `workspace/MEMORY.md` | Контекст | Любая |
| `starter_kit.md` / GitHub URL | Внедрение / эволюция kit | Любая |
| session-end | Принудительная консолидация памяти | Cursor: `.cursor/commands/`, OpenCode: `.opencode/commands/` (если среда поддерживает) |
| kit-sync | Sync с репозиторием kit | Как адаптер |
| kit-submit | Отправка learnings в issues kit | Как адаптер |
...
- [ ] `workspace/AUTONOMY.md`
- [ ] Правила безопасности (в адаптере платформы — `.cursor/rules/`, OpenCode-адаптер (если требуется), `mcp/`)
...
**Platform adapter (создаётся под среду):**

- [ ] Правила чтения GOALS/MEMORY + конституции
...
- [ ] `knowledge/INDEX.md`
- [ ] ≥1 проектный skill (в адаптере платформы)
...
| **1** | `CONSTITUTION.md` + `AUTONOMY.md` + правила безопасности и изоляции (в адаптере платформы) |
| **2** | Стадия 1 + файлы стадии 2 + корневой `AGENTS.md` + правило чтения GOALS/MEMORY |
| **3** | Стадия 2 + CONVENTIONS, TOOLS, ENV, SKILLS + `knowledge/INDEX.md` + ≥1 проектный skill (в адаптере платформы) |
...
| Проверка | OK |
|----------|-----|
| Путь верный (`workspace/` или адаптер платформы) | ☐ |
...
## decision_principles
- При конфликте: правила платформы → `workspace/CONSTITUTION.md` → `workspace/knowledge/`.
...
| Skill | Триггер | Путь |
|-------|---------|------|
| deploy | «задеплой», deploy | адаптер платформы/skills/deploy/SKILL.md |

## conflict_resolution
Проектный skill приоритетнее глобального (локальный адаптер > глобальный).
```
...
- skills: адаптер платформы/skills/
```
