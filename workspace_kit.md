...
| Авто-cron / Automations для outbound в kit | Непредсказуемость; push без согласия | Только по явной просьбе; агент + GitHub MCP |
...
| Направление | Кто | Как |
|-------------|-----|-----|
| **Kit → проект** | Агент в вашем проекте | Загрузить актуальный `workspace_kit.md` (raw URL или GitHub MCP `get_file_contents`); сверить версию с `workspace/INDEX.md`; предложить миграцию по [Протоколу эволюции](#протокол-эволюции-kit) |
| **Проект → kit** | Агент **только по вашей просьбе** | GitHub MCP: `search_issues` → дополнить существующую issue; иначе `issue_write` (create). **Без Python-скриптов** |

#### Что накапливается локально

Уроки и идеи — в обычных файлах workspace: `MEMORY.md`, `memory/`, `workspace/reports/`, `knowledge/`.  
Отдельный журнал «для kit» **не обязателен**: агент собирает универсальное из контекста, когда вы просите отправить feedback.
...
4. Применить **после вашего «да»** — не более 8 файлов за итерацию.
...
| Загрузить актуальный kit и мигрировать проект | Python sync-скрипты, manifest, cron push |
...
- При создании `workspace/INDEX.md` — записать `starter_kit_version: "1.10"` и `starter_kit_source: "local"` или `"remote"`
...
| **3** | + Конвенции языка | + check secrets/shell | ≥1 skill | kit-feedback |
...
| kit-feedback | Отправить универсальный опыт в issues workspace_kit (GitHub MCP); см. [Обратная связь](#обратная-связь-и-эволюция-kit-github-issues--mcp) | `.cursor/commands/` |
...
- [ ] `knowledge/INDEX.md`
- [ ] command `kit-feedback` (опционально)
- [ ] ≥1 skill в `.cursor/skills/`
- [ ] hooks: секреты + shell (audit-first)
- [ ] Правила конвенций языка
- [ ] GitHub MCP (`user-github`) доступен агенту — для feedback по просьбе
...
- [ ] Сверка `starter_kit_version` с GitHub — при новой версии kit предложить миграцию
- [ ] Feedback в kit — только по просьбе, через GitHub MCP
...
11. Нужен ли периодически отправлять универсальный опыт в kit (issues через GitHub MCP — только по просьбе)?
...
| GitHub MCP доступен для feedback в kit (стадия 3+, по просьбе) | ☐ |
...
4. Универсальный паттерн для kit → по просьбе пользователя: GitHub MCP (`search_issues` → comment или create)
...
### `.cursor/commands/kit-feedback.md` (стадия 3+, опционально)

```markdown
# kit-feedback

Отправить универсальный опыт workspace в [workspace_kit](https://github.com/riantdrew/workspace_kit) — **только по явной просьбе**.

Агент выполняет через **GitHub MCP** (`user-github`):

1. `search_issues` — найти похожие issues в `riantdrew/workspace_kit`
2. Если тема уже обсуждается — `add_issue_comment`
3. Если новая — `issue_write` (create) с предложением для `workspace_kit.md`
4. Не дублировать closed `not_planned`; анонимизировать (без домена, секретов, имён репо)

Источник: `MEMORY.md`, `memory/`, `reports/` — что накопилось локально.
