...
10. **MUST NOT** включать HEARTBEAT, CRON, `/loop` и произвольные Cursor Automations — вне scope kit. **kit-exchange** (стадия 3+) — через hook `sessionStart` + фоновый прогон агента, см. [kit-exchange по сессии](#kit-exchange-по-сессии).
...
|--------|------|-----------|
| **1.14** | 19.06.2026 | **kit-exchange по сессии:** вместо weekly Cursor Automation — `kit_exchange_last_run` в INDEX + проверка в `sessionStart` hook → агент запускает обмен в фоне при следующем чате (интервал по умолчанию 7 дней) |
...
| **1.11** | 19.06.2026 | **kit-exchange** (GitHub MCP): kit↔проект; в 1.14 — по сессии вместо weekly Automation |
...
| `/loop`, произвольные Cursor Automations | Непредсказуемая автономность | [kit-exchange по сессии](#kit-exchange-по-сессии) — hook + фоновый агент |
...
| Произвольный cron / push в kit | Риск без контроля | Только **kit-exchange** (≥1 раз/7д по сессии) + GitHub MCP |
...
| **Kit (внешний)** | `riantdrew/workspace_kit` | [kit-exchange](#kit-exchange-по-сессии) |
...
| Направление | Кто | Как |
|-------------|-----|-----|
| **Kit → проект** | Агент (по сессии, если срок, или `/kit-exchange`) | GitHub MCP: `get_file_contents` → `workspace_kit.md`; сверить версию; обновить `.cursor/` и `INDEX.md` |
| **Проект → kit** | Агент (по сессии, если срок, или `/kit-exchange`) | GitHub MCP: `search_issues` → comment; иначе `issue_write` (create). **Без Python-скриптов** |

#### Что накапливается локально

Уроки и идеи — в `MEMORY.md`, `workspace/<login>/memory/`, `workspace/reports/`, `knowledge/`.  
При **kit-exchange** агент собирает универсальное для feedback в kit (см. [B-шаг](#протокол-одного-прогона-kit-exchange) — режимы фоновый vs ручной).
...
| Еженедельный kit-exchange по сессии (hook + фон) | Python-скрипты, manifest, произвольный cron, Cursor Automations |

Зафиксировать в `workspace/AUTONOMY.md` → `never`: commit/push в репозиторий workspace_kit из другого prod-проекта.


#### kit-exchange по сессии

**Обязательно со стадии 3:** агент выполняет оба направления обмена **без Python-скриптов** и **без Cursor Automation** — через **GitHub MCP**, hook `sessionStart` и фоновый прогон при следующем чате.

| Параметр | Значение |
|----------|----------|
| Триггер | Новый чат, если с `kit_exchange_last_run` прошло ≥ `kit_exchange_interval_days` (по умолчанию **7**) |
| Механизм | `session_memory.py` в hook `sessionStart` → отметка `kit-exchange due` в daily memory → агент по BOOTSTRAP |
| Инструменты | GitHub MCP (**user-github**); отдельная Automation **не нужна** |
| Репозиторий | **этот** prod-проект (не workspace_kit) |
| Ручной дубль | `/kit-exchange` — тот же протокол в любой момент |

**Настройка (один раз, стадия 3+):**

1. Command `.cursor/commands/kit-exchange.md` — протокол A/B/C.
2. В `session_memory.py` (hook `sessionStart`): читать `kit_exchange_last_run` и `kit_exchange_interval_days` из `workspace/INDEX.md`; если срок — писать `kit-exchange due` в `workspace/<login>/memory/YYYY-MM-DD.md`.
3. В `workspace/BOOTSTRAP.md` → `session_start`: шаг «если due — запустить kit-exchange **в фоне**, не блокируя задачу пользователя».
4. В `.cursor/rules/workspace-read.mdc` (или аналог): то же правило для агента.
5. Зафиксировать в `workspace/AUTONOMY.md` → `always_allowed`: kit-exchange по сессии (фон) + ручной `/kit-exchange`.
6. Записать в `workspace/INDEX.md` (раздел `integrations`):

```text
kit_exchange_mode: on_session_if_due
kit_exchange_interval_days: 7
kit_exchange_last_run: —
```

7. Runbook (опционально): `workspace/knowledge/runbooks/kit-exchange-automation.md`.

**Проверка hook:**

```bash
python3 .cursor/hooks/session_memory.py session_start
# → JSON с kit_exchange_due: true/false
```
...
**MUST NOT в фоновом прогоне:** commit/push в workspace_kit; закрывать чужие issues; force-push; читать gitignored memory.

**Миграция с 1.11–1.13 (weekly Cursor Automation):** удалить или не создавать Automation; перенести на схему выше; заменить `kit_exchange_schedule: weekly` на `kit_exchange_mode: on_session_if_due`.
...
**Триггеры:** `starter_kit_version` в INDEX **старше** канона; `/kit-exchange` при расхождении версий; «обнови workspace», «накати kit 1.14», «мигрируй workspace».
...
2. **Git:** есть remote? private? Если нет — нужен ли сейчас (для Issues / kit-exchange)?
...
7. **kit-exchange по сессии:** включить hook + BOOTSTRAP (интервал 7д) или отложить?
...
| **< 1.11** | kit-exchange command + GitHub MCP | уже есть command |
...
| **→ 1.14** | kit-exchange по сессии вместо weekly Cursor Automation; `session_memory.py` + INDEX `kit_exchange_mode` | уже on_session_if_due |
...
- При создании `workspace/INDEX.md` — записать `starter_kit_version: "1.14"` и `starter_kit_source: "local"` или `"remote"`
...
| Фоновый kit-exchange читает gitignored memory | Нарушение границ B-шага | Фоновый режим: MEMORY + reports only |
...
workspace/
├── INDEX.md                # starter_kit_version, kit_exchange_mode/interval/last_run (стадия 4+)
├── BOOTSTRAP.md
├── SOUL.md
...
**Prerequisites:** GitHub MCP (**user-github**), `gh auth login`; kit-exchange по сессии (стадия 3+)
...
4. **kit-exchange due** (`sessionStart`, стадия 3+) — если с `kit_exchange_last_run` прошло ≥ interval_days, hook пишет отметку в daily memory; агент запускает обмен в фоне (см. [kit-exchange по сессии](#kit-exchange-по-сессии))
...
**Вне scope:** `/loop`, произвольные Cursor Automations и cron.
...
- [ ] command `kit-exchange` + [kit-exchange по сессии](#kit-exchange-по-сессии) (hook + фон)
...
- [ ] kit-exchange по сессии: `kit_exchange_last_run` актуален; hook возвращает `kit_exchange_due` корректно
...
11. Включить kit-exchange по сессии (hook `sessionStart`, интервал 7д, GitHub MCP)?
...
| kit-exchange по сессии настроен (стадия 3+): INDEX, hook, BOOTSTRAP | ☐ |
...
6. `git status`
7. **kit-exchange (фон):** если `kit_exchange_last_run` старше `kit_exchange_interval_days` (по умолчанию 7) — или в daily memory `kit-exchange due` — запустить `/kit-exchange` в фоне, не блокируя задачу пользователя
8. Опционально: «покажи задачи» → skill `gh-workspace`
...
4. Универсальный паттерн для kit → `/kit-exchange` или фоновый прогон при session_start
...
Двусторонний обмен с [workspace_kit](https://github.com/riantdrew/workspace_kit) — тот же протокол, что [kit-exchange по сессии](#kit-exchange-по-сессии).
...
**B. Фоновый (по сессии):** только `MEMORY.md` + `reports/` (не gitignored memory).

**C. Отчёт:** `workspace/reports/kit-exchange/YYYY-MM-DD.md` + `kit_exchange_last_run` в INDEX.md.

Репозиторий kit **не коммитить** из этого проекта.
```

### Расширение `session_memory.py` (kit-exchange due, стадия 3+)

В `cmd_session_start()`:

1. Прочитать `kit_exchange_last_run` и `kit_exchange_interval_days` из `workspace/INDEX.md`.
2. Если с last_run прошло ≥ interval_days (или last_run отсутствует) — записать в `workspace/<login>/memory/YYYY-MM-DD.md` → `in_progress`: `kit-exchange due — агент запускает в фоне`.
3. Вернуть JSON: `{ "ok": true, "kit_exchange_due": true|false, "kit_exchange_last_run": "YYYY-MM-DD" }`.

Агент по BOOTSTRAP и `workspace-read` rule запускает протокол kit-exchange в фоне, не блокируя ответ пользователю.

### `workspace/INDEX.md` — поля kit-exchange (стадия 3+)

```text
kit_exchange_mode: on_session_if_due
kit_exchange_interval_days: 7
kit_exchange_last_run: ДД.ММ.ГГГГ
```

**Устарело в 1.14:** `kit_exchange_schedule: weekly` — заменить на поля выше.
...
- **workspace-sync** и **kit-exchange по сессии** — отложить до `git init` + remote; repo указать в `PROJECT.md` / `workspace-sync.md` вручную
