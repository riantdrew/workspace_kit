...
Уроки и идеи — в обычных файлах workspace: `MEMORY.md`, `memory/`, `workspace/reports/`, `knowledge/`.  
Отдельный журнал «для kit» **не обязателен**: агент собирает универсальное из `MEMORY.md`, `memory/` (7 дней), `reports/` — при `/kit-exchange` или еженедельной Automation.

**В issues:** переносимые паттерны workspace/Cursor (hooks, rules, стадии, пробелы kit).  
...
3. Diff-план: новые rules, hooks, разделы workspace.
4. **Ручной запуск:** применить после «да». **kit-exchange Automation:** применить без запроса только `.cursor/commands/`, `.cursor/rules/` из kit-шаблонов и `starter_kit_version` в `INDEX.md`; **не** трогать SOUL, PROJECT, domain, CONSTITUTION, `.env`.

#### GitHub MCP (инструменты)

| Задача | MCP tool |
|--------|----------|
| Список issues | `list_issues` (`owner`: `riantdrew`, `repo`: `workspace_kit`) |
| Поиск | `search_issues` (`query`: `repo:riantdrew/workspace_kit …`) |
| Прочитать issue | `issue_read` |
| Комментарий | `add_issue_comment` |
| Создать / обновить | `issue_write` (`method`: create / update) |
| Прочитать kit | `get_file_contents` |
...
**Протокол одного прогона (агент выполняет целиком):**

```
A. Kit → проект
   1. get_file_contents → workspace_kit.md (riantdrew/workspace_kit)
   2. Сверить версию с workspace/INDEX.md → starter_kit_version
   3. Если remote новее — diff-план; применить безопасные обновления (.cursor/commands, .cursor/rules из kit-шаблонов)
   4. Обновить starter_kit_version, starter_kit_source: remote в INDEX.md
   5. Не перезаписывать: SOUL, USER, GOALS, MEMORY, PROJECT, knowledge/domain/, constitution rules, .env

B. Проект → kit
   1. Прочитать memory/ за 7 дней, MEMORY.md (lessons), workspace/reports/
   2. Выделить универсальные паттерны workspace/Cursor (без домена и секретов)
   3. Если нечего отправлять — пропустить с пометкой в отчёте
   4. search_issues (repo:riantdrew/workspace_kit) → comment или issue_write (create)
   5. Не дублировать closed not_planned

C. Отчёт
   workspace/reports/kit-exchange/YYYY-MM-DD.md + kit_exchange_last_run в INDEX.md
```

**MUST NOT в Automation:** commit/push в workspace_kit; закрывать чужие issues; force-push.
...
5. Предложить миграцию `workspace/` и `.cursor/`: новые rules, hooks, commands; обновление `starter_kit_version` и `starter_kit_source` в `INDEX.md`.
...
- При создании `workspace/INDEX.md` — записать `starter_kit_version: "1.11"` и `starter_kit_source: "local"` или `"remote"`
...
├── ENV.md
├── PLAYBOOK.md
├── reports/                # отчёты сессий, аудитов (опционально)
...
| **3** | + Конвенции языка | + check secrets/shell | ≥1 skill | kit-exchange |
...
Реализация: `session_memory.py` (single Python module) + shell-обёртки в hooks/ — **локальная** инфраструктура проекта, не часть kit repo.
...
| kit-exchange | Двусторонний обмен kit↔проект (GitHub MCP); см. [Обратная связь](#обратная-связь-и-эволюция-kit-github-issues--mcp) | `.cursor/commands/` |
...
- [ ] GitHub MCP (**github**) подключён для Automation и агента
...
- [ ] kit-exchange Automation отрабатывает ≥1 раз/нед; `kit_exchange_last_run` актуален
- [ ] Отчёты в `workspace/reports/kit-exchange/`
...
11. Настроить еженедельный kit-exchange Automation (понедельник 09:00, MCP github)?
...
4. Универсальный паттерн для kit → `/kit-exchange` или еженедельная Automation
...
**A. Kit → проект:** загрузить `workspace_kit.md`, сверить версию, обновить `.cursor/commands` / rules из kit, `starter_kit_version` в INDEX.md.

**B. Проект → kit:** за 7 дней из `memory/`, `MEMORY.md`, `reports/` — универсальные паттерны → GitHub MCP (`search_issues` → comment или `issue_write` create). Анонимизировать.
...
A. Kit → проект
- GitHub MCP: прочитать workspace_kit.md из riantdrew/workspace_kit (main)
- Сверить версию с workspace/INDEX.md → starter_kit_version
- Если на GitHub новее — обновить .cursor/commands и .cursor/rules по шаблонам из workspace_kit.md; обновить starter_kit_version и starter_kit_source: remote
- НЕ перезаписывать: workspace/SOUL.md, USER.md, GOALS.md, MEMORY.md, PROJECT.md, knowledge/domain/, constitution rules, .env

B. Проект → kit
- Прочитать memory/ (7 дней), MEMORY.md, workspace/reports/
- Универсальные паттерны workspace/Cursor → GitHub MCP: search_issues repo:riantdrew/workspace_kit; comment или create
- Анонимизировать: без домена, секретов, имён репо, /Users/...
- Пропустить, если нечего отправлять; не дублировать closed not_planned

C. Отчёт
- Записать workspace/reports/kit-exchange/YYYY-MM-DD.md
- Обновить kit_exchange_last_run в workspace/INDEX.md

НЕ commit/push в workspace_kit. НЕ закрывать issues.
```
